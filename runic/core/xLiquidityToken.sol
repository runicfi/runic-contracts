...
/**
 * @title xLiquidityToken
 * Locked liquidity token
 */
abstract contract xLiquidityToken is ERC20Burnable, AccessControlEnumerable, IERC20Burnable, IERC20Mintable {
    using SafeERC20 for IERC20;
    address liquidityOperator;

    error NotOperator();
    error NotLiquidityOperator();
    ...
    modifier onlyLiquidityOperator {
        if(msg.sender != liquidityOperator) revert NotLiquidityOperator();
        _;
    }

    constructor(string memory name, string memory symbol, address liquidityOperator_) ERC20(name, symbol) {
        ...
        liquidityOperator = liquidityOperator_;
    }
    ...
    function mint(address recipient_, uint256 amount_) public override onlyLiquidityOperator returns (bool) {
